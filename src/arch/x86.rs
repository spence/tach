#![cfg_attr(
  not(target_os = "linux"),
  allow(dead_code, reason = "Linux-only perf helpers are inert on other x86 targets")
)]

use core::arch::asm;
use core::arch::x86::{__cpuid, __cpuid_count, _rdtsc};
...
/// Brackets a TSC read on both sides for a perf userpage sequence-counter
/// read: prior metadata loads complete before the counter, and the closing
/// lock load cannot execute before it.
#[inline(always)]
#[cfg(feature = "thread-cpu-inline")]
...
fn detect_perf_seqlock_eligibility() -> OrderedReadEligibility {
  // SAFETY: the Rust i686 target guarantees CPUID and basic leaf 1.
  let basic = unsafe { __cpuid(0) };
  // SAFETY: basic leaf 1 is part of the i686 target contract.
...
fn read_cpuid_rdtsc_cpuid() -> u64 {
  // SAFETY: the i686 target guarantees CPUID and RDTSC. The intrinsics
  // preserve ABI-reserved registers and fully serialize both metadata sides.
  unsafe {
    let _ = __cpuid(0);
    let cycle = _rdtsc();
    let _ = __cpuid(0);
...
/// Reads TSC after prior instructions and loads have completed. See
/// `x86_64::rdtsc_ordered` for the selection and compiler-ordering rationale.
#[inline(always)]
...
pub(crate) fn read_mfence_rdtsc() -> u64 {
  let lo: u32;
  let hi: u32;
  // SAFETY: detection requires an AMD processor with SSE2. Every AMD CPU with
  // SSE2 implements AMD64, whose architecture specifies MFENCE as serializing.
  unsafe {
    asm!(
      "mfence",
      "rdtsc",
...
pub(crate) fn read_cpuid_rdtsc() -> u64 {
  let lo: u32;
  let hi: u32;
  // SAFETY: the i686 target guarantees CPUID. Saving EBX on the stack
  // preserves the register LLVM reserves for position-independent code. The
  // stack pointer is restored before leaving the block. CPUID fully serializes
  // prior instructions before RDTSC reads the counter.
  unsafe {
    asm!(
      "push ebx",
      "mov eax, 0",
...
fn detect_ordered_read_kind() -> u8 {
  // SAFETY: the i686 Rust target guarantees CPUID and leaf 0 is available.
  let basic = unsafe { __cpuid(0) };
  // SAFETY: the i686 target CPU has basic leaf 1.
  let leaf1 = unsafe { __cpuid(1) };
  // SAFETY: extended leaf 0 is the architected maximum-leaf query.
  let extended = unsafe { __cpuid(0x8000_0000) };
  let extended_edx = if extended.eax >= 0x8000_0001 {
...
pub(crate) fn bench_ordered_eligibility() -> (bool, bool, bool, bool) {
  // SAFETY: the i686 target guarantees the queried CPUID leaves.
  let basic = unsafe { __cpuid(0) };
  // SAFETY: the i686 target CPU has basic leaf 1.
  let leaf1 = unsafe { __cpuid(1) };
  // SAFETY: extended leaf 0 is the architected maximum-leaf query.
