...
| `W-WINDOWS` | QPC and available precise interrupt-time APIs | measured → fixed (M2) | native Windows x86_64 corroboration passes at `68dc201`; selected QPC beats `std` for both public wall APIs; windows-2025 and windows-2022 both select `windows_qpc` at `524b74a` — no same-target flip (`EVID-WINDOWS-FLIP`) → fixed QPC pick in M2 |
