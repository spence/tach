...
| Advertised target | `Instant` | `GlobalInstant` | `ThreadCpuInstant` |
|---|---|---|---|
...
| `W-MAC-X86` | XNU Mach absolute-time path (the invariant-TSC candidate was removed in M4) | fixed (M4) — `apple_mach_absolute_time` | **M4 (owner ruling):** `Instant` is a fixed `apple_mach_absolute_time` read. XNU's x86 `mach_timebase` is `1/1`, so that counter is already nanoseconds and is always available, needing no scale or runtime branch. The invariant-TSC branch was only ever selectable on a bare-metal Intel Mac (unavailable to this project) — an unvalidatable runtime branch — so it was dropped as a measured→fixed simplification, removing the last in-tree wall tournament for `Instant`. The bare-TSC path was source/codegen-proven but never performance-selected on retained evidence (the corrected 2026-07-15 note recorded `ineligible_cpuid_missing_invariant_tsc_or_rdtscp`, `tsc_eligible: false` on the only retained Intel env, github-macos-15-intel; it superseded the prior unsupported "selected invariant TSC" claim in `speed-supplemental-macos-x86_64.json`, `68dc201`). |
...
| `O-MAC-A64` | XNU Mach absolute/continuous timelines; eligible ordered architectural counter forms | measured (unresolved — `ESC-APPLE-ORDERED-SELECTION`) | **corrected 2026-07-16:** live observation on catalyst (M1 Max, `mode=3`, `cont_hwclock=1`) across 5 clean process starts shows the ordered selector picks `apple_mach_absolute_time` (3×) or `apple_mach_continuous_time` (2×) — the direct `isb`/`acntvct` counter routes reliably LOSE. This contradicts the prior "barriered `isb`+`CNTVCT` self-synchronizing route selected" claim, which `EVID-APPLE-BARE-CNTVCT` never recorded (it captured primitive read-costs, not a live ordered-selector observation — same unsupported-claim class as the corrected `W-MAC-X86` row). The cross-start coin-flip between the two Mach timelines is non-deterministic in sleep domain (absolute excludes sleep, continuous includes it), and `fallback::mach_time()` carries no `isb`, so whether the selected provider honors the `GlobalInstant` edge is open. Fixed-pick conversion is owner-blocked on `ESC-APPLE-ORDERED-SELECTION`; the ADR-0005 point that an unbarriered bare read is never an ordered candidate still holds. |
| `W-WINDOWS` | x86/x86_64 `Instant`: calibrated invariant TSC (same-core, ADR-0007) degrading to QPC; aarch64 `Instant`: QPC and available precise interrupt-time APIs | measured → fixed (M2); x86 `Instant` reopened under ADR-0007 (M4) | native Windows x86_64 QPC corroboration passed at `68dc201`; selected QPC beat `std` for both public wall APIs; windows-2025 and windows-2022 both selected `windows_qpc` at `524b74a` — no same-target flip (`EVID-WINDOWS-FLIP`) → QPC fixed in M2. **ADR-0007 (2026-07-17) relaxes the `Instant` cross-core requirement: x86/x86_64 `Instant` moves to a calibrated invariant TSC (`windows_tsc`) degrading to QPC when the invariant-TSC CPUID gate fails; aarch64 `Instant` and every `GlobalInstant` stay QPC. The x86 `Instant` speed cell awaits the M4 re-measure; the QPC evidence here still covers aarch64 `Instant` and all `GlobalInstant`.** |
| `O-WINDOWS` | the same Windows-owned APIs through their opaque call boundaries; raw TSC/CNTVCT and redundant pre-call fences are ineligible (class 1: Windows documents no userspace TSC invariance and designates QPC as the monotonic source — ADR-0005, upheld) | measured → fixed (M2) | 24-target optimized codegen closes at `5a2eb05`; focused proof found zero inversions for bare QPC in 945,307,669 reads, and native Windows x86_64 corroboration passes at `68dc201`; windows-2025 and windows-2022 both select `windows_qpc_call_boundary` at `524b74a` — no flip (`EVID-WINDOWS-FLIP`) → fixed in M2 |
