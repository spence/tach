...
| `W-MAC-X86` | XNU Mach absolute-time path; invariant TSC only after XNU-compatible eligibility and scale checks | measured | **corrected 2026-07-15:** on the only retained Intel environment (github-macos-15-intel, virtualized) the invariant TSC is INELIGIBLE (`tsc_eligibility_basis: ineligible_cpuid_missing_invariant_tsc_or_rdtscp`, `tsc_eligible: false`), so tach selects `apple_mach_absolute_time`. The bare-TSC wall path is source/codegen-proven but was never performance-selected on retained evidence (bare-metal Intel Mac unavailable). This replaces the prior unsupported "selected invariant TSC" claim (evidence `speed-supplemental-macos-x86_64.json`, `68dc201`) |
