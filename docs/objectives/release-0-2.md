...
mental model, final provider matrix, evidence SHA, and current toolchain facts.
...
source revision and evidence package; no public surface carries a stale target count or claim.
...
Pass: format, lint, tests, target proof, release evidence validation, `cargo package --locked`,
and `cargo publish --dry-run --locked` all pass against one candidate SHA.
