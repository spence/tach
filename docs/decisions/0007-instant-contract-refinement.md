# 0007 — Refined three-tier timer contract: same-core Instant, cross-core GlobalInstant
...
- **`GlobalInstant` — fastest cross-core-reliable clock.** For reading a timer across threads or a
...
forcing an apologetic public claim. The cross-core guarantee belongs to `GlobalInstant`: most
`Instant` use is same-core, and callers who cross cores/threads have `GlobalInstant`. Conflating
...
- `GlobalInstant` retains cross-core value consistency **and** the happens-before edge on every
...
- Windows `Instant`: `windows_qpc` → a raw-TSC provider. Windows `GlobalInstant` stays
...
- Only the Windows `Instant` speed cell re-measures; every `GlobalInstant` cell and the other
...
- Public claims reframe: `Instant` compared to the same-tier fast libraries (competitive),
  `GlobalInstant` to `std` (the only cross-thread public reference); no "slower but safer" caveat.
- The c7g `GlobalInstant` dispatch disposition (`fbe6e8b`, `ESC-APPLE-ELAPSED-DISPATCH`) is
  reinforced, not changed — `GlobalInstant` is explicitly the cross-core tier.
...
- Machine-checked by the speed campaign (`validate_campaign_for_checkout`): `Instant` cells
  competitive with the same-tier references; `GlobalInstant` cells beat `std` and carry the
